...
const Search = () => {
  ...
  const fetchTags = async () => {
    setError2("");
    try {
      const res: AxiosResponse<TagDto[]> = await HttpClient.get("/api/ManageTag");
      if (res?.data) {
        setTags(res?.data);
  ...
  const AddPlayCount = async (trackId: number) => {
    try {
      const res: AxiosResponse<Blob> = await HttpClient.get("/api/ManageTrack/plus-play-count", {
        params: {
          trackId: trackId,
        },
      });
  ...
  const fetchTracks = async (curr: number) => {
    ...
    try {
      const res: AxiosResponse<PagingResponseDto<TrackDto[]>> = await HttpClient.get(
        "/api/ManageTrack/get-range?currentPage=" + (curr - 1) + "&amount=" + itemsPerPage
      );
      console.log(res);
      if (res) {
      ...
      <Row className="search d-flex justify-content-center">
        <Col className="d-flex justify-content-end border-end left-col me-2" sm={3} xl={2}>
          <div className="tags text-end">
            ...
            <div>
              {error2 != "" ? (
                <div className="text-danger error2">{error2}</div>
              ) : (
                <div className="d-flex flex-column">
                  <FormControl
                    ...
                    onChange={(e) => {
                      setSearchTagValue(e.target.value.toLowerCase());
                    }}
                  />
        ...
        <Col xl={6} className="px-3 mt-5 right-col  mb-4">
          <div>
            <div className="search-title fw-bold">Tracks</div>
            <div className="search-result-title">
              ...
              <div className="mt-3 d-flex flex-wrap">
                {selectedTags.map((tag, idx) => (
                  <div
                    onClick={() => {
                      const filtered = selectedTags.filter((p) => p.Name != tag.Name);
          ...
          <div className="my-2 search-section d-flex align-items-center"></div>
                ...
                {tracks && tracks?.length > 0 ? (
                  <div className="search-body pt-2 d-flex flex-column">
                    {tracks.map((track, index) => (
                      <Row
                        className="track align-items-center border-bottom border-secondary"
                        key={index}
                        onClick={() => {
                          setSelected(track);
                          AddPlayCount(track.Id);
                        }}
                      >
                        ...
                        <Col className="d-flex justify-content-between">
                          <div className="desc1 d-flex flex-column">
                            {track.Id != undefined ? (
                              <Link to={"/music-detail/detail/" + track.Id} className="name title">
                                {track.TrackName}
                              </Link>
                            ) : (
                              <div className="name title">{track.TrackName}</div>
                            )}
                            <div className="mt-1 producer">
                              <FiPlay />
                              {track.PlayCount}
                            </div>
                          </div>
                            ...
                            {track.Tags.map((tag, idx) => (
                              <Tag
                                className={`py-2 me-2 ${
                                  ...
                                  selectedTags.find((p) => p.Name == tag.Name)
                                    ? "highlighted"
                                    : ""
                                }`}
                                name={tag.Name}
                  ...
                  <div className="search-empty text-center py-5">
                    <div className="title">No beats found :{"("}</div>
                    <div>{/* set popular beat here */}</div>
                  </div>
                )}
              </>
            )}
