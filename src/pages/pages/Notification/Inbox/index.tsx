...
const Inbox = () => {
  ...
  const setRead = async (item: MessageResponse) => {
    if (!item.IsReaded) {
      try {
        ...
        const res: AxiosResponse<MessageResponse[]> = await HttpClient.get(
          "/api/ManageNotification/read",
          {
            params: {
              userProfileId: userProfileId, //user?.userid,
        ...
        if (res) {
          if (notis) {
            const tempt = notis;
            tempt?.forEach((p) => {
              if (p.MessageId == item.MessageId) p.IsReaded = true;
              return true;
            });
            setNotis([...tempt]);
          }
  ...
  const setAllRead = async () => {
    try {
      ...
      const userProfileId = get.data.Id;
      const res: AxiosResponse<MessageResponse[]> = await HttpClient.get(
        "/api/ManageNotification/read-all",
        {
          params: {
            userProfileId: userProfileId, //user?.userid,
          },
  ...
  const FetchNotification = async () => {
    ...
    if (user?.userid != undefined) {
      try {
        ...
        const userProfileId = get.data.Id;
        const res: AxiosResponse<MessageResponse[]> = await HttpClient.get(
          "/api/ManageNotification/get-all",
          {
            params: {
              userProfileId: userProfileId, //user?.userid
            },
        ...
        if (res) {
          const tempt = [
            ...res.data
              .sort((a, b) => Date.parse(a.Message.CreatedDate) - Date.parse(b.Message.CreatedDate))
              .slice(0, 8),
          ];
          setNotis([...tempt]);
        }
  ...
  return (
    <Row className="mt-5">
      <Col xs="12">
        {isLoading ? (
          <div className="fs-1 fw-bold text-info text-center">LOADING</div>
        ) : (
          <div className="px-5">
