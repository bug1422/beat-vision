...
export const Message = (props: { value: TrackCommentDto }) => {
  ...
  const AddComment = async (message: string) => {
    try {
      ...
      if (user?.userid) data.append("userProfileId", userProfileId.toString()); //user?.userid);
      ...
      const res: AxiosResponse<TrackCommentDto[]> = await HttpClient.post(
        "/api/ManageComment/create-track-command-reply",
        data,
        {
          headers: {
  ...
  const FetchReplies = async () => {
    try {
      const res: AxiosResponse<TrackCommentDto[]> = await HttpClient.get(
        `/api/ManageComment/get-track-comments-reply?trackId=${props.value.TrackId}&commentId=${props.value.Id}`
      );
      if (res?.data) {
  ...
  const FetchUser = async () => {
    try {
      const res: AxiosResponse<UserProfileDto> = await HttpClient.get(
        "/api/ManageUser/identity/" + props.value.AuthorId
      );
      if (res?.data) {
        setUser(res?.data);
...
const TrackComment = (props: { trackId: number }) => {
  ...
  const AddComment = async (message: string) => {
    try {
      ...
      const res: AxiosResponse<TrackCommentDto[]> = await HttpClient.post(
        "/api/ManageComment/create-track-command",
        data,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  const FetchComment = async () => {
    try {
      const res: AxiosResponse<TrackCommentDto[]> = await HttpClient.get(
        "/api/ManageComment/get-track-comments",
        {
          params: {
            trackId: trackId,
          },
